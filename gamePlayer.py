...
from mcts import TreeNode,MonteCarloTreeSearch
...
class AI():
    ...
    def decision_making(self,level):
        ...
        position = self.gameboard.find_right_flipping_position(self.mark)
        for pos in self.pos_imt:
            if pos in position:
                return pos

        #生成一个虚拟的对手，进行训练
        opponent = AI(self.anti_mark,self.gameboard,self.ele_weights)
        ...
        node = TreeNode()
        iter_index = (int)(level/20)
        # print(iter_index)
        monte_carlo_tree_search = MonteCarloTreeSearch(self.mark,self.anti_mark,self.gameboard,self.size,self.ele_weights,node,self.iter[iter_index])
        next_step = monte_carlo_tree_search.search()
